...
std::pair<bool, std::wstring> Translate(const std::wstring& text)
{
	...
	if (firstTranslation) {
		...
		DevTools::SendRequest("Page.navigate", FormatString(LR"({"url":"https://translate.systran.net"})"));
		for (int retry = 0; ++retry < 100; Sleep(100))
			if (auto translation = Copy(DevTools::SendRequest("Runtime.evaluate",
				LR"({"expression":"document.querySelector('#translationTools').innerHTML.trim() ","returnByValue":true})"
			)[L"result"][L"value"].String())) if (!translation->empty()) break;
		DevTools::SendRequest("Runtime.evaluate", FormatString(LR"({"expression":"
			//document.waitForSelector('#target-dummydiv');
			document.querySelector('#cookieChoiceDismiss').click();
		"})"));
	}

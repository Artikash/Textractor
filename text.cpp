...
Clicca e trascina i bordi della finestra per muoverla, oppure nell'angolo in basso a destra per ridimensionare)";
    ...
    DICTIONARY_INSTRUCTIONS = u8R"(Questo file è utilizzato soltanto per la funzione "Dizionario" dell'estenzione Extra Window.
Utilizza un formato personalizzato specifico per Textractor e non è pensato per essere scritto manualmente.
Dovresti cercare online per un dizionario in questo formato (https://github.com/Artikash/Textractor-Dictionaries/releases è un buon posto per cominciare).
In alternativa, se sei un programmatore, puoi scrivere uno script per convertire un dizionario da un altro formato con le seguenti informazioni.
Una volta che hai ottenuto un dizionario, per guardare qualche testo in Extra Window, selezionalo. Tutte le definizioni corrispondenti saranno mostrate.
Le definizioni sono formattate in questo modo: |TERM|Hola|TERM|hola|TERM|Bonjour|TERM|bonjour|DEFINITION|hello|END|
La definizione può includere rich text (https://doc.qt.io/qt-5/richtext-html-subset.html) che saranno formattati correttamente.
Tutto il testo in questo file all'infuori di una definizione è ignorato.
Termini più lunghi di 50 caratteri potrebbero non essere visibili (per motivi di prestazioni che dovrebbero essere corretti a presto).
